          ...
          const l = on
            ? Object.keys(on).reduce((res, cur) => {
                ...
                if (typeof listener === 'function') {
                  ...
                  (component !== 'div' && component.name === listener.name) ||
                  (component === 'div' && listener.name === className)
                ) {
                  res[cur] = (type, ...args) => {
...
const createUseModal = Modal => () => {
  ...
  const openModal = async data => {
    modal = await Modal({ ...data, awaitClose: false });
  };
